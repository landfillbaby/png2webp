...
static bool p2wwrap(char *ip) {
  ...
  if(len > 3 && (u4(ip + len - 4) | u4("\0   ")) == u4(".png")) len -= 4;
#ifdef NOVLA
  char *op = malloc(len + 6);
  if(!op) {
    P("ERROR adding %s extension to %s: Out of memory", ".webp", ip);
    return 1;
  }
...
static bool w2pwrap(char *ip) {
  size_t len = strlen(ip);
  if(len > 4 && ip[len - 5] == '.'
      && (u4(ip + len - 4) | u4("    ")) == u4("webp"))
    len -= 5;
#ifdef NOVLA
  char *op = malloc(len + 5);
  if(!op) {
    P("ERROR adding %s extension to %s: Out of memory", ".png", ip);
    return 1;
  }
#elif defined __GNUC__
#pragma GCC diagnostic push
